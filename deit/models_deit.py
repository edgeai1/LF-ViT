...
from utils import batch_index_select, block_information
...
class MultiResoPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size_list=[112, 224], patch_size=16, in_chans=3, embed_dim=384):
        super().__init__()
...
class LFVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size_list=[112, 224], patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.alpha = 0.88
        # self.alpha = 1.0  79.48399995361328]
        self.beta = 0.99
        self.target_index = [3, 4, 5, 6, 7, 8, 9, 10, 11]
        self.patch_h = img_size_list[1] // patch_size
        self.patch_w = img_size_list[1] // patch_size

        self.image_size = img_size_list[1]
        self.region_size = region_size if self.image_size == 224 else 5
    ...
    def forward(self, xx):
        ...
        if self.informative_selection:
            ...
            important_index, unimportan_tokens = block_information(cls_attn, embedding_x1, self.alpha, self.region_size,
                                                                   self.image_size)
    ...
    def forward_early_exit(self, xx, threshold):
        ...
        x = self.norm(x)
        self.first_stage_output = x
        location_result = self.head(x[:, 0])
        logits_temp = F.softmax(location_result, 1)
        max_preds, _ = logits_temp.max(dim=1, keepdim=False)
        no_exit = max_preds < threshold

        # focus stage
        x = xx[1][no_exit]
        location_num = 1000 - x.shape[0]
        focus_num = x.shape[0]
        B = x.shape[0]
        x = self.patch_embed(x)
        cls_tokens = self.cls_token.expand(B, -1, -1)
        ...
        important_index, unimportan_tokens = block_information(cls_attn, embedding_x1[no_exit], self.alpha,
                                                               self.region_size, self.image_size)
        ...
        location_result[no_exit] = fine_result
