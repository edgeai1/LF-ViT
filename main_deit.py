...
def get_args_parser():
    ...
    parser.add_argument('--location-stage-size', default=8, type=int, help='the length of location splitting')
...
FFF    # Optimizer parameters
    ...
    parser.add_argument('--data-set', default='IMNET',
                        choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', 'IMNET10', 'IMNET100'],
...
def main(args):
    ...
    # checkpoint = torch.load('wookpoint['model'])
    if args.finetune:
    ...
    test_stats = evaluate(data_loader_val, model, device, args.input_size_list)
    print(
        f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats[f'{args.location_stage_size * 2 * 16}_acc1']:.1f}%")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(
            f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats[f'{args.location_stage_size * 2 * 16}_acc1']:.1f}%")
        if max_accuracy < test_stats[f'{args.location_stage_size * 2 * 16}_acc1']:
            shutil.copyfile(global_checkpoint_path, f'{args.output_dir}/model_best.pth')
        max_accuracy = max(max_accuracy, test_stats[f'{args.location_stage_size * 2 * 16}_acc1'])
